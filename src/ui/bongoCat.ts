...
export class BongoCatManager {
  ...
  mount() {
    ...
    container.className = `bongo-cat--Container ${this.getSizeClass(settings.bongoCatSize || 'medium')}`;
    ...
    // prevent touch scrolling while interacting with the widget
    container.style.touchAction = 'none';
    container.title = '点击页面任意位置或按键盘键让猫咪动起来！';

    container.innerHTML = this.svgHtml();
    // Prevent native dragging of the SVG which can open new tabs when dropped
    container.setAttribute('draggable', 'false');
    const svgEl = container.querySelector('svg');
    if (svgEl) {
      (svgEl as Element).setAttribute('draggable', 'false');
    }
    ...
    // global pointer handler so clicks anywhere (mouse/touch/pen) trigger paws
    const handleGlobalPointerDown = (ev: PointerEvent) => {
      if (ev.pointerType === 'touch') {
        this.lowerPaw('left');
        return;
      }
      if (ev.button === 0) this.lowerPaw('left');
      else if (ev.button === 2) this.lowerPaw('right');
    };
    ...
    (this as any)._listeners = { handleMouseDown, handleKeyDown, handleGlobalPointerDown };
  }

  unmount() {
    if (!this.container) return;
    const { handleMouseDown, handleKeyDown, handleGlobalPointerDown } = (this as any)._listeners || {};
    if (handleMouseDown) this.container.removeEventListener('mousedown', handleMouseDown);
    if (handleKeyDown) document.removeEventListener('keydown', handleKeyDown);
    if (handleGlobalPointerDown) document.removeEventListener('pointerdown', handleGlobalPointerDown);
  ...
  private getSizeClass(size: string) {
    switch (size) {
      case 'small': return 'bongo-cat--SizeSmall';
      case 'large': return 'bongo-cat--SizeLarge';
      default: return 'bongo-cat--SizeMedium';
    }
  }

  private onMouseDown(e: MouseEvent) {
    if (!this.container) return;
    // prevent native drag behavior (image/SVG dragging)
    e.preventDefault();
    // only start drag for left mouse button; other buttons should not drag
    if (e.button !== 0) {
      if (e.button === 2) this.lowerPaw('right');
      else this.lowerPaw('left');
      return;
    }

    // start drag
    const rect = this.container.getBoundingClientRect();
    // disable text selection during drag to avoid accidental editor save/refresh
    const prevUserSelect = document.body.style.userSelect;
    document.body.style.userSelect = 'none';
    // prevent selectstart (for older browsers / electron)
    ...
    const onMove = (ev: MouseEvent) => {
      if (!dragStarted) {
        const dx = Math.abs(ev.clientX - (rect.left + this.dragOffset.x));
        const dy = Math.abs(ev.clientY - (rect.top + this.dragOffset.y));
...
/* 尺寸设置 */
.bongo-cat--SizeSmall svg {
  height: 8vh;
  width: 10vw;
}

.bongo-cat--SizeMedium svg {
  height: 13vh;
  width: 15vw;
}

.bongo-cat--SizeLarge svg {
  height: 20vh;
  width: 22vw;
}
...
/* 为每条 path 指定具体动画名称（从原组件移植） */
#bongo-cat--f1-l1 { animation-name: bongo-cat--typing-f1-l1; }
#bongo-cat--f1-l2 { animation-name: bongo-cat--typing-f1-l2; }
#bongo-cat--f1-l3 { animation-name: bongo-cat--typing-f1-l3; }
#bongo-cat--f2-l4 { animation-name: bongo-cat--typing-f2-l4; }
#bongo-cat--f2-l5 { animation-name: bongo-cat--typing-f2-l5; }
#bongo-cat--f2-l6 { animation-name: bongo-cat--typing-f2-l6; }
#bongo-cat--f3-l7 { animation-name: bongo-cat--typing-f3-l7; }
#bongo-cat--f3-l8 { animation-name: bongo-cat--typing-f3-l8; }
#bongo-cat--f3-l9 { animation-name: bongo-cat--typing-f3-l9; }

@keyframes bongo-cat--typing-f3-l9 {
  0% { d: path("M8,25L8,25"); }
...
@keyframes bongo-cat--typing-f3-l8 {
  0% { d: path("M8,13L8,13"); }
...
@keyframes bongo-cat--typing-f3-l7 {
  0% { d: path("M0,1L0,1"); }
  60% { d: path("M0,1L0,1"); }
  68% { d: path("M0,1L96,1"); }
...
@keyframes bongo-cat--typing-f2-l6 {
  0% { d: path("M8,25L8,25"); }
...
@keyframes bongo-cat--typing-f2-l5 {
  0% { d: path("M8,13L8,13"); }
...
@keyframes bongo-cat--typing-f2-l4 {
  0% { d: path("M0,1L0,1"); }
  30% { d: path("M0,1L0,1"); }
...
@keyframes bongo-cat--typing-f1-l3 {
  0% { d: path("M8,25L8,25"); }
...
@keyframes bongo-cat--typing-f1-l2 {
  0% { d: path("M8,13L8,13"); }
...
@keyframes bongo-cat--typing-f1-l1 {
  0% { d: path("M0,1L0,1"); }
...
/* 放下状态时稍微向下移动，模拟按压效果 */
#bongo-cat--paw-right-down { transform: translateY(4px); }
#bongo-cat--paw-left-down { transform: translateY(4px); }

/* 键盘高亮效果 */
.bongo-cat--KeyboardActive #bongo-cat--laptop-keyboard { filter: brightness(1.2); transition: filter 0.075s ease; }
.bongo-cat--KeyboardActive #bongo-cat--laptop-keyboard polygon { fill: var(--interactive-accent) !important; }

/* 3D 变换效果 */
#bongo-cat--laptop-code { transform: rotateX(-37deg) rotateY(-46deg) rotateZ(-23deg) translateX(8px) translateY(20px) translateZ(-50px); }
#bongo-cat--laptop-cover { mix-blend-mode: hard-light; }

/* 深色主题样式 */
.theme-dark #bongo-cat--eyes, .theme-dark #bongo-cat--mouth path { stroke: #ffcc81; fill: #ffcc81; }
.theme-dark #bongo-cat--head-outline path:first-child, .theme-dark #bongo-cat--paw-left-up g path:first-child, .theme-dark #bongo-cat--paw-right-up g path:first-child, .theme-dark #bongo-cat--paw-left-down path:first-child, .theme-dark #bongo-cat--paw-right-down path:first-child { fill: #45465e; }
.theme-dark #bongo-cat--laptop-cover polygon, .theme-dark #bongo-cat--laptop-cover path { fill: #4f4f51e1; stroke: #0f0f10; }
.theme-dark #bongo-cat--laptop-terminal { stroke: #569cfa; fill: none; }
.theme-dark #bongo-cat--table path:first-child { fill: #45465e88; stroke: #45465e; }
.theme-dark #bongo-cat--laptop-base polygon { fill: #4f4f5188; stroke: #01070f; }
.theme-dark #bongo-cat--laptop-keyboard { stroke: #569cfa; fill: #569cfa; }

/* 浅色主题样式 */
#bongo-cat--head-outline path:first-child, #bongo-cat--paw-left-up g path:first-child, #bongo-cat--paw-right-up g path:first-child, #bongo-cat--paw-left-down path:first-child, #bongo-cat--paw-right-down path:first-child { fill: #fff; }
#bongo-cat--head-outline path:nth-child(2) { fill: #4f4f519f; }
`;
...
<svg id="bongo-cat" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 787.3 433.8">
        <defs>
          <symbol id="bongo-cat--eye" data-name="eye" viewBox="0 0 19.2 18.7">
          ...
          <symbol id="bongo-cat--paw-pads" data-name="paw-pads" viewBox="0 0 31.4 33.9">
        ...
        <g id="bongo-cat--head">
          ...
          <g id="bongo-cat--head-face">
            <g id="bongo-cat--eyes">
              <use width="19.2" height="18.7" transform="translate(474.8 195.2)" xlink:href="#bongo-cat--eye"></use>
              <use width="19.2" height="18.7" transform="matrix(-0.51, -0.85, 0.82, -0.5, 370.39, 192.59)" xlink:href="#bongo-cat--eye"></use>
            </g>
            <g id="bongo-cat--mouth">
              <path d="M450.2,198.3c.6,1.2.2,1.9-.2,2.2a36.7,36.7,0,0,1-7.6,4.9,14.9,14.9,0,0,1-4.8,1.4h-1.4l-1.3-.2-1.4-.4-1.3-.6a21.6,21.6,0,0,1-6.4-7.2,52.8,52.8,0,0,1-4-8.3l3.8,1.3a62.3,62.3,0,0,1-7.1,4.1,32.1,32.1,0,0,1-7.9,2.8,13.2,13.2,0,0,1-4.9.2l-1.4-.3a7.5,7.5,0,0,1-1.3-.6,7.9,7.9,0,0,1-2.3-1.6,16.8,16.8,0,0,1-2.9-4,24.1,24.1,0,0,1-1.6-4.2c-.1-.5,1.6-1.3,3-1.4s3.5.2,3.6.6a10.3,10.3,0,0,0,2.6,4.9l.7.5h2.4l1.5-.2a28.4,28.4,0,0,0,6.5-2c2.1-1,4.3-2.1,6.3-3.3h.1a2.5,2.5,0,0,1,3.4.9l.3.5a43.1,43.1,0,0,0,3.2,7.7,19.8,19.8,0,0,0,2.2,3.4,8.1,8.1,0,0,0,2.6,2.6,5,5,0,0,0,3.7-1,33.4,33.4,0,0,0,7.2-4.3C448.8,197.4,449.5,197.2,450.2,198.3Z"></path>
            </g>
          </g>
        </g>
        <g id="bongo-cat--table">
        ...
        <g id="bongo-cat--laptop">
          ...
          <g id="bongo-cat--laptop-keyboard" class="${this.keyboardActive ? 'bongo-cat--KeyboardActive' : ''}">
          ...
          <g id="bongo-cat--paw-right-down" style="display:none">
          ...
          <g id="bongo-cat--laptop-terminal">
          ...
          <g id="bongo-cat--laptop-terminal-code-scene">
            <g id="bongo-cat--laptop-code">
              <g stroke="#3DE0E8" stroke-width="6" transform="matrix(-1 0 0 1 278 103)">
                <g id="bongo-cat--f3" transform="translate(0 76)">
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f3-l9" d="M8,25L8,25" stroke-dasharray="60,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f3-l8" d="M8,13L8,13" stroke-dasharray="50,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f3-l7" d="M0,1L0,1" stroke-dasharray="25,10"></path>
                </g>
                <g id="bongo-cat--f2" transform="translate(0 38)">
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f2-l6" d="M8,25L8,25" stroke-dasharray="40,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f2-l5" d="M8,13L8,13" stroke-dasharray="60,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f2-l4" d="M0,1L0,1" stroke-dasharray="30,10"></path>
                </g>
                <g id="bongo-cat--f1">
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f1-l3" d="M8,25L8,25" stroke-dasharray="60,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f1-l2" d="M8,13L8,13" stroke-dasharray="60,10"></path>
                  <path class="bongo-cat--typing-animation" id="bongo-cat--f1-l1" d="M0,1L0,1" stroke-dasharray="60,10"></path>
                </g>
              </g>
            </g>
          </g>
          <g id="bongo-cat--laptop-cover">
        ...
        <g id="bongo-cat--paw-left-down" style="display:none">
