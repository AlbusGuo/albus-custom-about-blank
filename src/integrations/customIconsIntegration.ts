...
} from "src/integrations/customIconsApi";
...
export class CustomIconsIntegration {
  ...
  private requiredIconIds: string[] = [];
  ...
  async openIconPicker(
    ...
    if (
      initialIcon
      && (initialIcon.startsWith("CI-") || !initialIcon.includes(":"))
    ) {
  ...
  renderIcon(element: HTMLElement, iconId: string): boolean {
    ...
    try {
      const shouldResync = this.api !== api;
      this.adoptApi(api);
      if (shouldResync) {
        void this.syncRequiredIcons(this.requiredIconIds).catch(() => undefined);
      }
  ...
  syncRequiredIcons(iconIds: string[]): Promise<void> {
    ...
    this.requiredIconIds = this.normalizeIconIds(iconIds);
    this.pendingIconIds = [...this.requiredIconIds];
  ...
  private adoptApi(api: AlbusCustomIconsApi): void {
    if (this.destroyed || this.api === api) {
      return;
    }
    this.unsubscribe?.();
    this.api = api;
